...
public class Grid : MonoBehaviour
{
    ...
    public float cameraBoundsOffset = 4f;
    ...
    private void Create()
    {
        ...
        PanAndZoom.boundMaxY = YLength - cameraBoundsOffset;
        PanAndZoom.boundMinX = 0f;
        PanAndZoom.boundMinY = -cameraBoundsOffset;
    ...
    public bool CanPutObject(GridObject gridObject, bool checkBuildings = true)
    {
        for (int x = gridObject.gridPosition.x; 
            x < gridObject.gridPosition.x + gridObject.xLength; x++)
        {
                ...
                y < gridObject.gridPosition.y + gridObject.yLength; y++)
            {
                if (x >= xLength || y >= yLength || x < 0 || y < 0
                    || (checkBuildings && gridObjects[x, y] != null))
                {
                    return false;
                }
            }
    ...
    /// <returns></returns>
    public bool CanPutObject(GridObject gridObject, GridCoordinates updaetdCoordinates, bool checkBuildings = true)
    {
